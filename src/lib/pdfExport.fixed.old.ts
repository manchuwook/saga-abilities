...
const formatAbilitiesTable = (doc: jsPDF, abilities: Spell[]) => {
  const tableColumns = [
  ...
  const tableRows = abilities.map(spell => ({
...
export const exportAbilitiesToPDF = (abilities: Spell[]) => {
  ...
  // Detailed spell descriptions - two abilities per page
  for (let i = 0; i < abilities.length; i += 2) {
    if (i + 1 < abilities.length) {
      // Add two abilities per page when possible
      addSpellDetails(doc, abilities[i], false);
      addSpellDetails(doc, abilities[i + 1], true);
    } else {
...
const addAbilityManualCoverPage = (doc: jsPDF, AbilityManual: AbilityManual) => {
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  const spellCountText = `Contains ${AbilityManual.abilities.length} abilities`;
...
const addTableOfContents = (doc: jsPDF, abilities: Spell[]) => {
  doc.addPage();
...
export const exportAbilityManualToPDF = (AbilityManual: AbilityManual) => {
  ...
  for (let i = 0; i < AbilityManual.abilities.length; i += 2) {
    if (i + 1 < AbilityManual.abilities.length) {
      // Add two abilities per page when possible
      addSpellDetails(doc, AbilityManual.abilities[i], false);
      addSpellDetails(doc, AbilityManual.abilities[i + 1], true);
    } else {
      // Add single spell if we have an odd number
      addSpellDetails(doc, AbilityManual.abilities[i], false);
    }
